...
func GetTableMetaInfo(options *Options) ([][]*TableMeta, error) {
	...
	dataFiles, err := os.ReadDir(options.DataDirPath)
	if err != nil {
...
func findKeyFromTable(lookup *LookupKey, meta *TableMeta) (bool, []byte, []byte, error) {
...
func searchTableMap(lookup *LookupKey, tm []*TableMeta, L0 bool) (bool, []byte, []byte, error) {
	if L0 {
		for _, meta := range tm {
			if meta.Compare(lookup.GetInternalKey()) == 0 {
				exist, key, value, err := findKeyFromTable(lookup, meta)
				if err != nil || exist {
					return exist, key, value, err
				}
			}
		}
	} else {
		...
		for l < r {
			meta = tm[(l+r)/2]
			cmp := meta.Compare(lookup.GetInternalKey())
			if cmp < 0 {
				r = (l+r)/2 - 1
			} else if cmp > 0 {
				l = (l+r)/2 + 1
		...
		return findKeyFromTable(lookup, meta)
	}
	return false, nil, nil, nil
}
...
func (z *DB) Get(key []byte) ([]byte, error) {
	...
	for level, tm := range z.tableMeta {
		...
		exist, _, value, err = searchTableMap(lookup, tm, level == 0)
		if err != nil {
			if errors.Is(err, ErrDataDeleted) {
				return nil, ErrKeyIsNotExist
			}
